...
function ToggleShowHide(str)
{
	// reverse all props listed in str
	var ckstr = "";
	for (var n = 0; n < CondAttrs.length; n++) {
		var aname = CondAttrs[n];
		var avalue = aname[1];
		for (var v = 0; v < avalue.length; v++) {
			ckstr = aname[0] + "_" + avalue[v][0];
			if (str.indexOf(ckstr) != -1)
				avalue[v][1] = avalue[v][1] ? 0 : 1;
		}
	}
...
function ShowElem(elem, over)
{
	// remove display: none if present in style
	if (over) {// remove red color if any
...
function HideElem(elem, over)
{
	// add display: none if not present in style
	if (over) {  // color red to indifste hidden
		if (elem.style && elem.style.color) {
			if (elem.style.color != "red")
